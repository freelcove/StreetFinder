      ...
      if (session && session.accessToken) {
        fetch(`${backendUrl}/coordinates`, {
          ...
          .then((data) => {
            ...
            const initializeMap = () => {
              const mapOptions = {
                center: new window.naver.maps.LatLng(data.latitude, data.longitude),
                zoom: 10,
              };
              ...
              const centerMarker = new window.naver.maps.Marker(centerMarkerOptions);
              window.naver.maps.Event.addListener(map, "click", function (e: any) {
                ...
                if (latInputRef.current && lngInputRef.current) {
                  ...
                  const startLatLng = new window.naver.maps.LatLng(data.latitude, data.longitude);
  ...
  return (
    <div style={{ display: "flex" }}>
      ...
      <div style={{ width: "30%", position: "relative" }}>
        ...
        <div style={{ textAlign: "left" }}>{distance && <p>Distance: {distance.toFixed(2)} km</p>}</div>
      </div>
    </div>
  );
};
